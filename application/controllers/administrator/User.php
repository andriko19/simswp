	...
	*/
	public function index($offset = 0)
	{
        ...
        if ($a == 1) {
        	...
        	$this->data['users'] = $this->model_user->get_siswa_smk($filter, $field, $this->limit_page, $offset);
			$this->data['user_counts'] = $this->model_user->count_all_siswa_smk($filter, $field);

			$config = [
				'base_url'     => 'administrator/user/index/',
				'total_rows'   => $this->model_user->count_all_siswa_smk($filter, $field),
				'per_page'     => $this->limit_page,
				'uri_segment'  => 4,
			];
        } // filter admin sma
        else if ($a == 15) {
        	$this->data['users'] = $this->model_user->get_siswa_sma($filter, $field, $this->limit_page, $offset);
			$this->data['user_counts'] = $this->model_user->count_all_siswa_sma($filter, $field);

			$config = [
				'base_url'     => 'administrator/user/index/',
				'total_rows'   => $this->model_user->count_all_siswa_sma($filter, $field),
				'per_page'     => $this->limit_page,
				'uri_segment'  => 4,
			];
		} // filter admin smp
        else if ($a == 14) {
        	$this->data['users'] = $this->model_user->get_siswa_smp($filter, $field, $this->limit_page, $offset);
			$this->data['user_counts'] = $this->model_user->count_all_siswa_smp($filter, $field);

			$config = [
				'base_url'     => 'administrator/user/index/',
				'total_rows'   => $this->model_user->count_all_siswa_smp($filter, $field),
				'per_page'     => $this->limit_page,
				'uri_segment'  => 4,
			];
		}
	...
	public function guru($offset = 0)
	{
		...
		$this->data['users'] = $this->model_user->get_guru($filter, $field, $this->limit_page, $offset);
		$this->data['user_counts'] = $this->model_user->count_all_guru($filter, $field);

		$config = [
			'base_url'     => 'administrator/user/guru/',
			'total_rows'   => $this->model_user->count_all_guru($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	public function siswa($offset = 0)
	{
		...
		$this->data['users'] = $this->model_user->get_siswa($filter, $field, $this->limit_page, $offset);
		$this->data['user_counts'] = $this->model_user->count_all_siswa($filter, $field);

		$config = [
			'base_url'     => 'administrator/user/siswa/',
			'total_rows'   => $this->model_user->count_all_siswa($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	public function guru_smp($offset = 0)
	{
		$this->is_allowed('user_list');
		...
		$this->data['users'] = $this->model_user->get_guru_smp($filter, $field, $this->limit_page, $offset);
		$this->data['user_counts'] = $this->model_user->count_all_guru_smp($filter, $field);

		$config = [
			'base_url'     => 'administrator/user/guru_smp/',
			'total_rows'   => $this->model_user->count_all_guru_smp($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	public function siswa_smp($offset = 0)
	{
		$this->is_allowed('user_list');
		...
		$config = [
			'base_url'     => 'administrator/user/siswa_smp/',
			'total_rows'   => $this->model_user->count_all_siswa_smp($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	public function guru_sma($offset = 0)
	{
		$this->is_allowed('user_list');
		...
		$this->data['users'] = $this->model_user->get_guru_sma($filter, $field, $this->limit_page, $offset);
		$this->data['user_counts'] = $this->model_user->count_all_guru_sma($filter, $field);

		$config = [
			'base_url'     => 'administrator/user/guru_sma/',
			'total_rows'   => $this->model_user->count_all_guru_sma($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	public function siswa_sma($offset = 0)
	{
		$this->is_allowed('user_list');
		...
		$config = [
			'base_url'     => 'administrator/user/siswa_sma/',
			'total_rows'   => $this->model_user->count_all_siswa_sma($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	public function guru_smk($offset = 0)
	{
		$this->is_allowed('user_list');
		...
		$this->data['users'] = $this->model_user->get_guru_smk($filter, $field, $this->limit_page, $offset);
		$this->data['user_counts'] = $this->model_user->count_all_guru_smk($filter, $field);

		$config = [
			'base_url'     => 'administrator/user/guru_smk/',
			'total_rows'   => $this->model_user->count_all_guru_smk($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	public function siswa_smk($offset = 0)
	{
		$this->is_allowed('user_list');
		...
		$config = [
			'base_url'     => 'administrator/user/siswa_smk/',
			'total_rows'   => $this->model_user->count_all_siswa_smk($filter, $field),
			'per_page'     => $this->limit_page,
			'uri_segment'  => 4,
		];
	...
	*/
	public function edit_save($id)
	{
		...
		if ($this->form_validation->run()) {
			$user_avatar_uuid = $this->input->post('user_avatar_uuid');
			$user_avatar_name = $this->input->post('user_avatar_name');
      $user_avatar_old = $this->input->post('user_avatar_old');
			...
			if (!empty($user_avatar_name)) {
				...
				// 	// $user_avatar_name_copy = date('YmdHis') . '' . $user_avatar_name;
					...
					rename(FCPATH . '/uploads/user/' . $user_avatar_uuid . '/' . $user_avatar_name,
							FCPATH . '/uploads/user/' . $user_avatar_name_copy);



        //   // $upload_image = $_FILES['user_avatar_name'];
        ...
        //   //   if ($this->upload->do_upload($user_avatar_name_copy)) {
        //   //     $save_data['avatar'] = $user_avatar_name_copy;
        //   //   } else {
        //   //     return $this->response([
        //   //       'success' => false,
        ...
        //   // }
				...
				// 	// 	$file_name = $user_avatar_name_copy;
				...
				// 	// 	if ($this->upload->do_upload('user_avatar_name')) {

				// 	// 		// $data['error'] = $this->upload->display_errors();

				// 	// 		$uploaded_data = $this->upload->data();

				// 	// 		$save_data['avatar'] = $uploaded_data['file_name'];
