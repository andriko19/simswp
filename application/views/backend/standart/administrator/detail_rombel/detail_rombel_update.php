...
<section class="content-header">
    <h1>
        Detail Rombel        <small>Edit Detail Rombel</small>
    </h1>
    <ol class="breadcrumb">
                ...
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                        ...
                        <div class="widget-user-header ">
                            ...
                            <h5 class="widget-user-desc">Edit Detail Rombel</h5>
                            <hr>
                        </div>
                        <?= form_open(base_url('administrator/detail_rombel/edit_save/'.$this->uri->segment(4)), [
                            ...
                            <div class="col-sm-8">
                                <input type="number" class="form-control" name="a" id="a" placeholder="A" value="<?= set_value('a', $detail_rombel->a); ?>">
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select-deselect" name="id_rombel" id="id_rombel" data-placeholder="Select Id Rombel" >
                                    <option value=""></option>
                                    <?php foreach (db_get_all_data('rombel') as $row): ?>
                                    <option <?=  $row->id_rombel ==  $detail_rombel->id_rombel ? 'selected' : ''; ?> value="<?= $row->id_rombel ?>"><?= $row->nama_rombel; ?></option>
                                    <?php endforeach; ?>  
                                </select>
                                <small class="info help-block">
                                </small>
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select-deselect" name="id_siswa" id="id_siswa" data-placeholder="Select Id Siswa" >
                                    ...
                                    <option <?=  $row->id_siswa ==  $detail_rombel->id_siswa ? 'selected' : ''; ?> value="<?= $row->id_siswa ?>"><?= $row->nama; ?></option>
                                    <?php endforeach; ?>  
                                </select>
                                <small class="info help-block">
                                </small>
...
<script>
    $(document).ready(function(){
      ...
      $('.btn_save').click(function(){
        ...
        $.ajax({
          url: form_detail_rombel.attr('action'),
          type: 'POST',
          dataType: 'json',
          data: data_post,
        })
        .done(function(res) {
          if(res.success) {
            var id = $('#detail_rombel_image_galery').find('li').attr('qq-file-id');
            if (save_type == 'back') {
