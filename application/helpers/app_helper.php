...
if(!function_exists('db_get_data')) {
	function db_get_data($table_name = null, $where = null) {
		$ci =& get_instance();
		if ($where) {
			$ci->db->where($where);
		}
...
if(!function_exists('display_menu_admin_role')) {
	function display_menu_admin_role($parent, $level) {
		$ci =& get_instance();
		$ci->load->database();
		$ci->load->model('model_menu');
	    $result = $ci->db->query("SELECT a.id, a.label,a.icon_color, a.type, a.link,a.icon, Deriv1.Count FROM `menu` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 1 AND a.parent=" . $parent." order by `sort` ASC")->result();
		...
		if(!empty($ci->input->get('role')))
		{
			$ci->session->set_userdata('groups', $ci->input->get('role'));
			$ci->session->set_userdata('ngroups', $ci->input->get('rname'));
			
		} 
		$groups=$ci->session->groups;
		$ngroups=$ci->session->ngroups;
	    	...
	    	if (($level > 1) AND ($parent > 0) ) {
		   	...
		   	foreach ($result as $row) {
				...
				$segments = array_slice($ci->uri->segment_array(), 0, count($links));
		   		...
		   		if ($row->type == 'label') {
		   			if (($ci->aauth->is_allowed($perms)) and ($ci->aauth->is_group_allowed($perms,$groups)))  {
		        		$ret .= '<li class="header">'._ent($row->label).'</li>';
		        	}
		   		} else {
			        if ($row->Count > 0) {
			        	if (($ci->aauth->is_allowed($perms)) and ($ci->aauth->is_group_allowed($perms,$groups)))  {
							...
							$ret .= display_menu_admin_role($row->id, $level + 1);
							$ret .= "</li>";
						}
			        } elseif ($row->Count==0) {
			           if (($ci->aauth->is_allowed($perms)) and ($ci->aauth->is_group_allowed($perms,$groups)))  {
...
if(!function_exists('display_menu_admin_by_groups')) {
	function display_menu_admin_by_groups($parent, $level) {
		$ci =& get_instance(); 
		$ci->load->database();
		$ci->load->model('model_menu');
		//$role=$ci->aauth->get_user_groups($ci->aauth->get_user()->id);
		$ret = '';
		//foreach ($role as $rl) {
	    $result = $ci->db->query("SELECT a.id, a.label,a.icon_color, a.type, a.link,a.icon, Deriv1.Count FROM `menu` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 1 AND a.parent=" . $parent." order by `sort` ASC")->result();
		//$groups = $rl->id; 
		...
		$ret .= '<i class="fa fa-sign-in"></i>'.$ngroups;
		//$ret .= '<h3 class="box-title">'.$rl->id." ".$rl->name.'</h3>';
	    if ($result) {
	    	if (($level > 1) AND ($parent > 0) ) {
		    	$ret .= '<ul class="treeview-menu">';
	    	} 
			// else {
	    		// $ret = '';
	    	// }
			//echo '<h3 class="box-title">'.$rl->name.'</h3>';
		   		...
		   		if ($row->type == 'label') {
			        ...
			        if ($row->Count > 0) {
			        	if (($ci->aauth->is_allowed($perms)) and ($ci->aauth->is_group_allowed($perms,$groups)))  {
							...
							$ret .= display_menu_admin_by_groups($row->id, $level + 1);
							$ret .= "</li>";
						}
			        } elseif ($row->Count==0) {
			           if (($ci->aauth->is_allowed($perms)) and ($ci->aauth->is_group_allowed($perms,$groups)))  {
